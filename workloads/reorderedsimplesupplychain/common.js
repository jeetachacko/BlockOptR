...
let txnum = 10000;
let workernum = 10;
let txperworker = Math.floor(txnum/workernum);
let allfuncs = ["pushASN", "ship", "queryASN", "unload", "updateAuditInfo", "queryProducts"];
let criticalfuncs = [0, 1, 2, 3];
let otherfuncs = [4, 5]
let txnum_otherfuncs = [200, 25];
let funcindexchangecount = Math.floor(txperworker/(criticalfuncs.length * 2))
let functionname = 0;
let locations = ["A", "B", "C", "D"];
let auditkey = 0;
let reorderworkerstartindexx = 0; //Current worker pointer index
let reorderworkerstartindexy = 0; //Current worker pointer index
let reordercount = 1;
...
class CreateWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
	    ...
	    if (this.txIndex == 0) {
		    ...
		    reorderworkerstartindexx = this.workerIndex * txperworker;
		    reorderworkerstartindexy = this.workerIndex * txperworker;
		    workerstartindex = this.workerIndex * txperworker;
		    workertxcount = workerstartindex;
		    workerfirstcount = workerstartindex;
	    }
	    else if (this.txIndex > (txperworker - txnum_otherfuncs[1])) {
		    argindex = otherfuncs[1]
		    workertxcount = reorderworkerstartindexx
		    reorderworkerstartindexx++
		    if ((reorderworkerstartindexx % funcindexchangecount) == 0) {
			    reordercount = funcindexchangecount * criticalfuncs.length 
			    reorderworkerstartindexx = (this.workerIndex * txperworker) + reordercount;
		    }

	    }
	    else if (this.txIndex > (txperworker - (txnum_otherfuncs[0] + txnum_otherfuncs[1]))) {
		    argindex = otherfuncs[0]
		    workertxcount = reorderworkerstartindexy
                    reorderworkerstartindexy++
		    if ((reorderworkerstartindexy % funcindexchangecount) == 0) {
			    reordercount = funcindexchangecount * criticalfuncs.length
                            reorderworkerstartindexy = (this.workerIndex * txperworker) + reordercount;
                    }
	    ...
	    if (workertxcount < 3000) {
		    startkey = workertxcount + 3000
		    endkey = startkey + 3
	    }

	    auditkey = "AK" + (Math.floor(Math.random() * 1000)).toString()
	    ...
	    let funcargs = [
                ...
                {contractId : 'simplesupplychain', contractFunction: 'UpdateAuditInfo', contractArguments: [(workertxcount).toString(), auditkey.toString()], readOnly: false, timeout: 120}, 
