...
class ElectronicHealthRecordContract extends Contract {
    ...
    async revokeProfileAccess(ctx, ssn, key, actorId) {
        ...
        const hashedKey = sha512(key);
        if (hashedKey === profile.hashedKey) {
            const profstatus = profile.revokeProfileAccess(hashedKey, actorId);
	    if (profstatus == 1) {
		    const buffer = Buffer.from(JSON.stringify(profile));
		    await ctx.stub.putState(ssn, buffer);
		    return profile;
		    ...
		    console.log("Profile access was never granted")
		    throw new Error("Profile access was never granted");
		    //return {} 
    ...
    async revokeEHRAccess(ctx, ssn, ehrId, key, actorId) {
	    ...
	    const ehrstatus = profile.revokeEHRAccess(hashedKey, actorId, ehrId);
		console.log("ehrstatus")
		console.log(ehrstatus)
	    if (ehrstatus == 1) {
		    const buffer = Buffer.from(JSON.stringify(profile));
		    await ctx.stub.putState(ssn, buffer);
		    ...
		    console.log("EHR access was never granted")
		    throw new Error("Profile access was never granted");
                    //return {}
                }
