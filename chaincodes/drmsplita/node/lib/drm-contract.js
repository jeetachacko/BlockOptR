...
class DrmContract extends Contract {
	...
	let chaincodeResponse = await ctx.stub.invokeChaincode("drmsplitb", ["InitLedger"], ctx.stub.getChannelID());
	//let newResp = JSON.parse(chaincodeResponse.payload.toString('utf8'));
	  //   console.log("chaincodeResponse is: " + newResp);
    ...
    async create(ctx, metadataId, metadata, royaltyManagementId, royaltyManagement) {
	    ...
	    const chaincodeResponse = await ctx.stub.invokeChaincode("drmsplitb", ["create", metadataId, metadata, royaltyManagementId, royaltyManagement], ctx.stub.getChannelID());
        }
    }
