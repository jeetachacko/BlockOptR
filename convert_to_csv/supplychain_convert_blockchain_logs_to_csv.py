    ...
    for _ , file_path in ordered_directory_dict.items():
        ...
        with open(file_path) as json_file:
            ...
            try:
                ...
                res_len = len(json_data['data']['data'])
                for x in range(res_len):
                    arg_len = len(res[x]['payload']['data']['actions'][0]['payload']['chaincode_proposal_payload']['input']['chaincode_spec']['input']['args'])
                    print(arg_len)
                    newrestr = ''
                    for y in range(1, arg_len):
                        arg_new = res[x]['payload']['data']['actions'][0]['payload']['chaincode_proposal_payload']['input']['chaincode_spec']['input']['args'][y]['data']
                        restr = bytes(arg_new).decode("utf8")
                        if "Time" not in restr:
                            newrestr = newrestr + ' ' + restr
                            print(restr)
                    field_values.append(newrestr)
            except Exception:
                field_values.append("NULL")
                pass
